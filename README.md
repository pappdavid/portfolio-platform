...
- **Auth layer** — Clerk handles all authentication, organisation management, RBAC, and billing. The middleware (`src/proxy.ts`) protects `/dashboard` routes. Navigation items are filtered client-side by organisation role via `useFilteredNavItems()`.
...
### Public Site
- **Landing page** — animated hero, project pillar cards, terminal quickstart demo, theme switcher
- **Product pages** — `/mcp`, `/training`, `/chat` — standalone marketing pages per project with architecture diagrams
- **Project detail pages** — `/projects/{slug}` — deep-dive write-ups with tech stack, architecture SVGs, and links
...
- **Referral system** — `/r/[token]` redirect handler, Supabase-backed event tracking
- **HMAC signing** — MCP API key verification via `MCP_HMAC_SECRET`
- **Multi-theme** — six CSS variable themes in OKLCH format, zero-flash cookie persistence
...
All events — pass and block — are written to Supabase (`mcp_events` table) and surfaced in the dashboard with guard outcome, timestamp, and payload hash. API keys are HMAC-signed and stored with RLS enforced per user.
...
A retrieval-augmented generation chat interface with a conditional Three.js rendering layer. The pipeline:

1. **Ingestion** — documents are chunked, embedded via `text-embedding-3-small`, and stored in Supabase with `pgvector`
2. **Retrieval** — hybrid search: dense vector similarity + sparse keyword match, re-ranked by cross-encoder score
3. **Generation** — retrieved chunks are injected as context; citations are enforced in the system prompt
4. **3D layer** — if the response contains spatial/graph data, a Three.js scene renders conditionally alongside the text response

Unauthenticated users hit `chatRateLimit` (10/hr); authenticated users hit `chatAuthRateLimit` (50/hr). The Thesys C1 component handles the streaming UI.

**Stack:** OpenAI API (embeddings + chat) · Supabase pgvector · Upstash Redis · Three.js · Thesys C1
...
│       ├── mcp-sentinel/page.tsx   # Project detail → /projects/mcp-sentinel
│       ├── rag-chat/page.tsx       # Project detail → /projects/rag-chat
│       ├── training/page.tsx       # Project detail → /projects/training
...
│   ├── training/route.ts           # Training job dispatch
│   ├── training/datasets/route.ts  # Dataset management
│   └── ref/route.ts                # Referral tracking
├── r/[token]/page.tsx              # Referral redirect handler
├── about/page.tsx
...
| Components | [Aceternity UI](https://ui.aceternity.com) | Spotlight, beams, hover-border-gradient effects |
...
| 3D rendering | [Three.js](https://threejs.org) | Conditional spatial data visualisation in the chat UI |
| Chat UI | [Thesys C1](https://thesys.so) | Streaming generative UI component |
...
Six tables in Supabase, all with RLS enabled:
...
Migration SQL: [`supabase/migrations/001_initial.sql`](./supabase/migrations/001_initial.sql)
...
| `OPENAI_API_KEY` | Optional | Enables the RAG chat feature |
...
├── styles/
│   └── themes/             # 6 CSS variable theme files (OKLCH)
...
Six themes are included, toggled via the dashboard settings or the landing page theme switcher:

| Theme | Description |
|---|---|
| `vercel` | Default dark — black background, white/zinc palette |
...
Themes are applied via `data-theme` on `<html>` and persisted in an `active_theme` cookie to avoid flash on first render.
